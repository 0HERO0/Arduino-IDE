...
#include <array> // array 자료구조 = 배열
#include <sys/socket.h>
#include <arpa/inet.h> //192.168.0.2 -> change network 값으로 바꿔주는 라이브러리
#include <pthread.h>
...
int main(int argc, const char* argv[])
{
	...
	if(serial_port == -1)
	{
		std::cout << "Serial cpnnet error()" << std::endl;
		exit(EXIT_FAILURE);
	}
	...
	while(true)
	{
	    ...
	    std::cout << "Connect from client IP : " <<
	    ...
	    //클라이언트들 Thread 만들기
	    pthread_detach(pid); // 자동 종료 -> pthread_join();
	}
...
void* taskSerial(void* arg)
{
	...
	if(!serial_state)
	{
	    ...
	    {
		write(fd,serial_message.data(), strlen(serial_message.data())); // ->> ))) 3개??
	    }
